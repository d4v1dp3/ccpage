...
if (!empty($_POST)) {
    ...
    if (!isset($hidden)) {
        ...
        $registro = 'false';
        try {
            $statement = $conexion->prepare("UPDATE conferencia SET nombre_conferencia=?,descripcion=?,nombre_ponente=?,cupo_maximo=?,fecha_curso=STR_TO_DATE(?,'%d/%m/%Y %H:%i'),fecha_termino_curso=STR_TO_DATE(?,'%d/%m/%Y %H:%i'),lugar=? WHERE id=?");
            $statement->bind_param("ssssssss", $nombre, $descripcion, $ponente, $cupo, explode('-',$fecha_evento)[0],explode('-',$fecha_evento)[1], $lugar, base64_decode($datos[0]));
            $statement->execute();
