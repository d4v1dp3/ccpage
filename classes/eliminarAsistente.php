...
if (!empty($_POST)) {
    ...
    if (!isset($hidden)) {
        ...
        $_SESSION['toastr'] = "toastr.error('', 'No se han recibido parametros');";
    } elseif ($consultar->eliminarUsuario(base64_decode($datos[0])) == 'false') {
        $_SESSION['toastr'] = 'toastr.error("", "La solicitud no pudo ser completada, intente nuevamente");';
    } else {
        $_SESSION['toastr'] = 'toastr.success("", "Se han eliminado los registros del asistente <i>' . $datos[1] . '</i>");';
    }
} else {
    $_SESSION['toastr'] = 'toastr.error("", "La solicitud no pudo ser completada, intente nuevamente");';
}
header('Location: ../admin/asistentes.php');
?>